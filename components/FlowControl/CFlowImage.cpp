...
CFlowImage::CFlowImage(const char* logTag)
{
	this->logTag = logTag;
	isLogImage = false;
    disabled = false;
...
CFlowImage::CFlowImage(std::vector<CFlow*> * lfc, const char* logTag) : CFlow(lfc)
{
	this->logTag = logTag;
	isLogImage = false;
    disabled = false;
...
CFlowImage::CFlowImage(std::vector<CFlow*> * lfc, CFlow *_prev, const char* logTag) :  CFlow(lfc, _prev)
{
	this->logTag = logTag;
	isLogImage = false;
    disabled = false;
...
string CFlowImage::CreateLogFolder(string time) {
	if (!isLogImage)
		return "";
...
void CFlowImage::LogImage(string logPath, string name, float *resultFloat, int *resultInt, string time, CImageBasis *_img) {
	if (!isLogImage)
		return;
